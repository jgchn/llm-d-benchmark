  ...
  for model in ${LLMDBENCH_DEPLOY_MODEL_LIST//,/ }; do
    ...
    if [[ $LLMDBENCH_HARNESS_SKIP_RUN -eq 1 ]]; then
      ...
      if [[ $LLMDBENCH_CONTROL_ENVIRONMENT_TYPE_MODELSERVICE_ACTIVE -eq 1 ]]; then
        ...
        export LLMDBENCH_HARNESS_STACK_ENDPOINT_PORT=80
        echo $LLMDBENCH_HARNESS_STACK_ENDPOINT_NAME
      fi
      ...
      if [[ $LLMDBENCH_CONTROL_ENVIRONMENT_TYPE_MODELSERVICE_ACTIVE -eq 1 ]]; then
        export LLMDBENCH_HARNESS_STACK_ENDPOINT_URL="http://${LLMDBENCH_HARNESS_STACK_ENDPOINT_NAME}-istio${LLMDBENCH_VLLM_FQDN}:${LLMDBENCH_HARNESS_STACK_ENDPOINT_PORT}/${LLMDBENCH_DEPLOY_CURRENT_MODELID}"
      else
